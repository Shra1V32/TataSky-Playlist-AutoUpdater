...
if sys.argv[1]!='':
    ...
    current_date=requests.get("http://worldtimeapi.org/api/timezone/Asia/Kolkata")
    a_year, a_month, a_day = current_date.json()["datetime"][:10].split('-')
    a_hours, a_minutes, a_seconds = current_date.json()["datetime"][11:19].split(':')
    ...
    ts= (datetime.datetime(int(a_year),int(a_month),int(a_day),int(a_hours),int(a_minutes),int(a_seconds)) - datetime.datetime(int(u_year),int(u_month),int(u_day),int(u_hours),int(u_minutes),int(u_seconds))).total_seconds()
    if int(ts) < 1209600:
